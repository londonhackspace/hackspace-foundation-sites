...
<h4>New members in <?=date('F')?>, go up and say hi!</h4> 
    ...
    $users = $db->translatedQuery( "SELECT id, photo, full_name FROM users JOIN users_profiles ON (id=user_id) WHERE photo != '' AND users.disabled_profile = 0 ORDER BY lower(full_name)"); ?>

...
<h4>New members in <?=date('F')?>, go up and say hi!</h4> 
        ...
        foreach( $newUsers as $user ) { 
			if($user['photo'] != null && $user['photo'] != '') { ?>
            	<a href="/members/profile/<?=$user['user_id']?>"><img style="max-width: 80px;" src="/members/photo.php?name=<?=$user['photo'] ?>&amp;size=med" alt="<?=$user['full_name'] ?>" title="<?=$user['full_name']?>"/></a>
        <?	}
        } 
    } ?>

<h2>The faces of London Hackspace.</h2> 
        ...
        foreach( $users as $user ) { ?>
            	<a href="/members/profile/<?=$user['id']?>"><img style="max-width: 80px;padding-bottom:5px;" src="/members/photo.php?name=<?=$user['photo'] ?>&amp;size=med" alt="<?=$user['full_name'] ?>" title="<?=$user['full_name']?>"/></a>
        <? } ?>
